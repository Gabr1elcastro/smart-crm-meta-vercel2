...
const Plans: React.FC = () => {
  ...
  // Proteção: usuários Trial e Plus podem acessar esta página
  useEffect(() => {
    if (!loading && clientInfo) {
      const hasTrialFeatures = clientInfo.trial === true || clientInfo.plano_plus === true;
      if (!hasTrialFeatures) {
        // Se não está no Trial/Plus, redirecionar para o dashboard
        navigate('/');
      }
    }
  ...
  const plans: Plan[] = [
    {
      id: 'pro-agencias',
      name: 'Pro',
      description: 'Plano único com atendimento em tempo real integrado',
      price: {
        monthly: 1997.0,
      },
      features: [
  ...
  const handlePlanSelection = async () => {
    ...
    try {
      const checkoutLink = 'https://www.asaas.com/c/r0d7pygzrk6stwid';
  ...
  const getCurrentPlan = () => {
    ...
    if (clientInfo.plano_agentes || clientInfo.plano_pro || clientInfo.plano_starter) return 'premium';
    return null;
  };
  ...
  // Se não está no Trial/Plus, não renderizar nada (será redirecionado pelo useEffect)
  if (!clientInfo || (!clientInfo.trial && !clientInfo.plano_plus)) {
    return null;
  }
              ...
              >
                ...
                <CardContent className="flex flex-col h-full">
                  ...
                  <div className="pt-6 mt-auto">
                    {clientInfo?.plano_plus ? null : isCurrentPlan ? (
                      <Button 
                        variant="outline" 
