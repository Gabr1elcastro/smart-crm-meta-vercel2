...
import { Loader2, Check, X, Zap, Star, Settings } from 'lucide-react';
...
export default function RdStationConfigModal({ onClose }: RdStationConfigModalProps) {
  const { toast } = useToast();
  const [funisRd, setFunisRd] = useState<FunilRD[]>([]);
  const [selectedFunilId, setSelectedFunilId] = useState<number | null>(null);
  const [etapas, setEtapas] = useState<EtapaFunilRD[]>([]);
  ...
  const loadFunis = async () => {
    try {
      setIsLoading(true);
      console.log('🔍 Carregando funis RD...');
      ...
      if (data && data.length > 0) {
        ...
        toast({
          title: "Aviso",
          description: "Nenhum funil foi encontrado. Conecte primeiro um token do RD Station.",
          variant: "destructive"
        });
        onClose();
      }
      ...
      toast({
        title: "Erro",
        description: "Erro ao carregar funis do RD Station",
        variant: "destructive"
      });
      onClose();
    } finally {
  ...
  const handleSelectFunil = async (funilId: number) => {
    ...
    try {
      ...
      const funil = funisRd.find(f => f.id === funilId);
      if (!funil) return;
      ...
      return (
        <div className="text-center py-8">
          ...
          <p className="text-gray-600">
            Buscando funis disponíveis no RD Station.
          </p>
        </div>
      );
    }
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-gray-600 text-sm mb-4">
                Selecione qual funil do RD Station será usado como padrão para novos leads.
              </p>
            </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500">ID: {funil.id_funil_rd}</p>
                        </div>
                      </div>
                      {funil.funil_padrao && (
      ...
      <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Configurar Funil RD Station</h3>
