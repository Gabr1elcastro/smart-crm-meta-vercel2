...
interface AudioRecorderAACProps {
...
export const AudioRecorderAAC: React.FC<AudioRecorderAACProps> = ({
  onSendAudio,
  onCancel,
  isRecording,
  onStartRecording,
  ...
  const startMediaRecording = async () => {
    try {
      ...
      console.log('🎤 Iniciando gravação de áudio para WhatsApp');
      
      // Detectar o melhor formato suportado
      const formats = [
        { mime: 'audio/mp4', ext: 'mp4' },        // Melhor para WhatsApp
        { mime: 'audio/aac', ext: 'aac' },        // AAC direto
        { mime: 'audio/webm;codecs=opus', ext: 'webm' }, // Fallback
        { mime: 'audio/webm', ext: 'webm' }       // Último recurso
      ];
      
      let selectedFormat = formats[formats.length - 1]; // Default
      
      for (const format of formats) {
        if (MediaRecorder.isTypeSupported(format.mime)) {
          selectedFormat = format;
          console.log(`✅ Formato selecionado: ${format.mime}`);
          break;
        }
      }
      ...
      mediaRecorder.onstop = () => {
        ...
        const audioBlob = new Blob(chunksRef.current, { type: selectedFormat.mime });
        
        console.log('✅ Áudio gravado:', {
          formato: selectedFormat.mime,
          extensão: selectedFormat.ext,
          tamanho: `${(audioBlob.size / 1024).toFixed(2)} KB`,
          duração: `${recordingTime}s`,
          tipoBlob: audioBlob.type
        });
        
        // Adicionar extensão ao blob para ajudar na identificação
  ...
  const handleSend = async () => {
    ...
    try {
      console.log('📤 Enviando áudio para WhatsApp:', {
        ...
        extensão: (audioBlob as any)._extension
      });
  ...
  if (audioBlob && audioUrl) {
    ...
    const formatType = audioBlob.type.includes('mp4') ? 'MP4/AAC' : 
