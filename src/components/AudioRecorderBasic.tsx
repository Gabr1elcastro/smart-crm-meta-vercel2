...
interface AudioRecorderBasicProps {
...
export const AudioRecorderBasic: React.FC<AudioRecorderBasicProps> = ({
  onSendAudio,
  onCancel,
  isRecording,
  onStartRecording,
    ...
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ 
        audio: {
          ...
          sampleRate: 16000, // Taxa menor para compatibilidade
          channelCount: 1    // Mono
        } 
      });
      ...
      console.log('🎤 Iniciando gravação básica de áudio');
      
      // Usar APENAS WebM que é garantido funcionar no navegador
      ...
      const mediaRecorder = new MediaRecorder(stream, {
        mimeType: mimeType
        // Removendo audioBitsPerSecond para deixar o navegador decidir
      });
      ...
      // Iniciar gravação com timeslice para garantir chunks regulares
      mediaRecorder.start(1000); // Gerar chunk a cada 1 segundo
  ...
  const handleSend = async () => {
    ...
    setIsSending(true);
    try {
      console.log('📤 Enviando áudio WebM básico');
  ...
  const handleCancel = () => {
    ...
    // Garantir que o stream está parado
    if (streamRef.current) {
      streamRef.current.getTracks().forEach(track => track.stop());
      streamRef.current = null;
    }
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-green-600 mt-1">
              {(audioBlob.size / 1024).toFixed(0)} KB • WebM
            </p>
          </div>
          <Button
            variant="ghost"
