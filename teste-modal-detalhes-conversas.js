// Script para testar o modal de detalhes do contato na página de conversas
// Execute este script no console do navegador quando estiver na página /conversations

console.log('=== TESTE: Modal de Detalhes do Contato - Conversas ===');
...
// 3. Verificar se o menu de contexto está funcionando
function verificarMenuContexto() {
  console.log('🔍 Verificando menu de contexto...');
  
  const contextMenus = document.querySelectorAll('[class*="ContextMenu"]');
  console.log('Menus de contexto encontrados:', contextMenus.length);
  
  if (contextMenus.length === 0) {
    console.log('❌ Nenhum menu de contexto encontrado');
    return false;
  }
...
// 4. Função para simular clique com botão direito em um contato
...
function verificarOpcoesMenu() {
  ...
  let encontrouAbrirConversa = false;
  ...
  opcoes.forEach((opcao, index) => {
    ...
    if (texto.includes('Abrir conversa')) {
      encontrouAbrirConversa = true;
      console.log('✅ Opção "Abrir conversa" encontrada');
    }
  });
...
function verificarEstrutura() {
  ...
  const elementos = {
    'Lista de contatos': document.querySelector('[class*="w-full md:w-1/3"]'),
    'Área de conversa': document.querySelector('[class*="flex-1 min-w-0"]'),
    'Campo de mensagem': document.querySelector('[id="mensagem-input"]')
  };
...
console.log('🚀 Iniciando testes do modal de detalhes...');
...
if (verificarContatos()) {
  // Verificar menu de contexto
  if (verificarMenuContexto()) {
    ...
    setTimeout(() => {
      ...
      console.log('3. Verifique se o modal abre com as informações do contato');
      ...
      console.log('✅ Menu de contexto com botão direito');
      console.log('✅ Opção "Ver detalhes" no menu');
      console.log('✅ Modal com informações do contato');
      console.log('✅ Busca automática de informações do lead');
      console.log('✅ Exibição de status de atendimento');
      console.log('✅ Exibição de etiquetas (se disponível)');
      console.log('✅ Interface responsiva e acessível');
    }, 3000);
  } else {
    console.log('❌ Menu de contexto não encontrado');
  }
} else {
  console.log('');
